...
int signMessage(const char *message, const char *privateKeyPath) {
  ...
  // The key must be in PEM format. Notice that OpenSSH uses a different pem
  // format. You can convert an OpenSSH key to PEM with the following command:
  ...
  std::string signature_base64 = hex_encode(signature, signatureLen);
